...
import (
	"context"
	"errors"
	v1 "github.com/38888/nunu-layout-advanced/api/v1"
...
func (s *userService) Register(ctx context.Context, req *v1.RegisterRequest) error {
	// check username
	user, err := s.userRepo.FindOneByOther(ctx, &model.User{Email: req.Email})
...
func (s *userService) GetProfile(ctx context.Context, userId int64) (*v1.GetProfileResponseData, error) {
	user, err := s.userRepo.FindOneByOther(ctx, &model.User{UserId: userId})
	if err != nil {
		return nil, err
	}
